...
class LogisticRegression(GeneralModel):
    ...
    def fit(self, X, y):
        ...
        self.model = LogisticRegressionModule(features_num=k_num)
        ...
        while self.loss > 0.000001 and self.epoch_num < self.epochs_max_num:
            for X_train_batch, y_train_batch in self._get_minibatch(x_train, y_train, self.batch_size):
                ...
                loss = nn.BCEWithLogitsLoss()(y_pred.reshape(y_pred.shape[0]), y_train_batch)
                loss.backward()
                self.optimizer.step()
